...
function createSimpleAudio() {
    try {
        ...
        const createSound = (type, duration, frequency, gain = 0.2) => {
            return () => {
                try {
        ...
        // Función para crear un sonido más complejo
        const createComplexSound = (config) => {
            return () => {
                try {
                    ...
                    for (let i = 0; i < config.layers.length; i++) {
                        ...
                        gain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + layer.duration);
                        ...
                        osc.start();
                        osc.stop(audioContext.currentTime + layer.duration);
                    }
                } catch (error) {
                    console.warn("Error al reproducir sonido complejo:", error);
                }
            };
        };
        ...
        return {
            hit: createSound('square', 0.1, 200, 0.15),
            pickup: createSound('sine', 0.2, 800, 0.15),
            levelup: createComplexSound({
                layers: [
                    { type: 'sawtooth', frequency: 400, gain: 0.1, duration: 0.2 },
                    { type: 'sine', frequency: 600, gain: 0.1, duration: 0.3 },
                    { type: 'triangle', frequency: 800, gain: 0.1, duration: 0.4 }
                ]
            }),
            stairs: createComplexSound({
                layers: [
                    { type: 'triangle', frequency: 400, gain: 0.1, duration: 0.4 },
                    { type: 'sine', frequency: 600, gain: 0.05, duration: 0.5 }
                ]
            }),
            enemyDeath: createSound('square', 0.3, 100, 0.15),
            playerDeath: createComplexSound({
                layers: [
                    { type: 'sawtooth', frequency: 150, gain: 0.1, duration: 0.5 },
                    { type: 'sine', frequency: 100, gain: 0.1, duration: 0.7 },
                    { type: 'square', frequency: 50, gain: 0.1, duration: 1.0 }
                ]
            })
        };
    } catch (error) {
